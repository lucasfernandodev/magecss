...
export async function getList() {
  ...
  const convertToArray = respTags && Array.from(respTags).map((item) => {
  ...
  const populationPostsInList = convertToArray && convertToArray.map(async item => {
    item.content = await getPostToListBySlug(item.slug);
    return item;
  })

  const resultado = populationPostsInList && await Promise.all(populationPostsInList);
  return resultado
}
